 ...
 */
var wsServer = 'ws://114.215.154.238:9502';

var websocket = new WebSocket(wsServer);
websocket.onopen = function (evt) {
    console.log("别没事就打开什么firbug,看啥看啊！");
};
...
;!function(win, undefined){
    ...
    var config = {
        brief: false,
        msgurl: '234234',
        chatlogurl: 'http://wwww.iphpt.com/',//聊天记录地址
        aniTime: 200,
        right: -232,
        api: {
            ...
            groups: '/group.php', //群组成员接口
            sendurl: ''
    //发送消息接口
        },
        user: { //当前用户信息
            name: '游客' + Math.ceil(Math.random()*10000),
            face: 'https://odu38kv7q.qnssl.com/nice.png'
        },
        uploadImage: {
            url: '', //接口地址（返回的数据格式见下文）
            type: 'post' //默认post
...
//聊天窗口
    xxim.popchat = function(param){
        ...
        log.html = '<div class="layim_chatbox" id="layim_chatbox">'
            ...
            +'        <a href="http://www.iphpt.com" target="_blank" class="layim_seechatlog"><i class="fa fa-comment-o"></i>聊天记录</a>'
            +'    </div>'
...
//定位到某个聊天队列
    xxim.tabchat = function(param){
        ...
        xxim.chatbox.find('.layim_seechatlog').attr('href', config.chatlogurl );//此处是跳转聊天记录页面的
...
//消息传输
    xxim.transmit = function(){
        ...
        log.send = function(){
            ...
            websocket.send(JSON.stringify(selectData));
            ...
            if(data.content.replace(/\s/g, '') === ''){
                ...
                websocket.onmessage = function (evt) {
                    ...
                    if (config.user.name != jQuery.parseJSON(evt.data).userName) {
                        log.imarea.append(log.html({
                            time: myDate.toLocaleString(),
                            name: jQuery.parseJSON(evt.data).userName,
                            face: jQuery.parseJSON(evt.data).userFace,
                            content: jQuery.parseJSON(evt.data).content
                        }));
                        // node.imwrite.val('').focus();
                        log.imarea.scrollTop(log.imarea[0].scrollHeight);
                    }
